...
function envoy_on_request(request_handle)
    bearer = request_handle:headers():get("authorization")
    -- remove bearer prefix
    bearer = bearer:gsub("%bBearer ", "")
    bearer = bearer:gsub("%bbearer ", "")
    if jwt == nil then
        request_handle:logInfo("empty authorization")
    else
        content = decode_jwt(jwt)
        claims = content["claims"]
        header = content["head"]
        for k,v in pairs(header) do
            request_handle:logInfo(strings.format("header: %s = %s", k, v))
        end
        for k,v in pairs(claims) do
            request_handle:logInfo(strings.format("claim: %s = %s", k, v))
        end
    end
end
